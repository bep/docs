...
```file {title="/etc/conf.d/fcgiwrap"}
...
```file {title="/etc/nginx/conf/sites-available/www.example.com"}
server {
    listen   80;
...
```file {title="/srv/http/example.com/public\\_html/test.cgi"}
#!/usr/bin/perl

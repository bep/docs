...
Before continuing, ensure that you've completed the ProxyPass guide, particularly the section regarding [configuring the proxy module](/docs/guides/multiple-web-servers-with-proxypass-on-fedora-14/). Do not omit to create and configure the `/etc/httpd/conf.d/proxy.conf` file.

Once `mod_proxy` is enabled and [configured properly](/docs/guides/multiple-web-servers-with-proxypass-on-fedora-14/), you can insert the following directives into your virtual hosting configuration.
